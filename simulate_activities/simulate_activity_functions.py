...
import sys
sys.path.append("..")
from scripts.surface import predict_energies, initiate_surface, adsorb_CO, adsorb_H, adsorb_NO, block_indices
from scripts.methods import CO_NO_pairs_energy
...
def simulate_activity(composition,P_CO,P_NO, metals, p_react, eU=0, n=100, max_iter=int(1e+5),time_interval=1000,rate_time=10,tol=1e-6, return_history=False):
    t_start = time()
    #Initiate surface
    ...
    if np.all(CO_energies>0) or np.all(NO_energies>0):
        rate = 0
        print(str(np.around(composition,decimals=4)),"Reaction rate",rate,"evaluation time (s):",time()-t_start)
        return rate
    ...
    for iteration in range(max_iter):
        ...
        if n_pairs>0:
            ...
            reaction_bool = p_reaction <= p_react
        ...
        if (iteration+1)%time_interval==0:
            D_reactions= d_reactions/time_interval
            d_reactions=0

            D_reactions_list.append(D_reactions)
            it_list.append(iteration)
            
            if iteration>=time_interval*rate_time:
                
                # Get rate as moving average
                rate = np.mean(D_reactions_list[-rate_time:])
                # Linear regression
                a,*ignore = linregress(it_list[-rate_time:], D_reactions_list[-rate_time:])
                ...
                if abs(a)<(rate*tol):
                    ...
                    if return_history:
                        return D_reactions_list, it_list, rate
                    else:
                        return rate
