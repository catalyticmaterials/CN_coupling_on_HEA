...
#print(H_coverage_mask)
#print(NO_coverage_mask)
...
for iteration in tqdm(range(iterations)):
        ...
        if H_blocked: #continue
            ...
            if Delta_E<0 or np.random.rand() <= np.exp(-Delta_E/kBT):
                ...
                for i_b,j_b in NO_blocking_ind:
                    ...
                    not_H_blocked = np.array([np.any(H_coverage_bool[block_indices(ib_, jb_, "top").T])==False for ib_,jb_ in zip(i_ub,j_ub)])
    ...
    if n_pairs>0:
        ...
        if len(CO_NO_pair_reacted)>0:
            ...
            for (i_CO,j_CO,i_NO,j_NO) in CO_NO_pair_reacted:
                ...
                not_H_blocked = np.array([np.any(H_coverage_bool[block_indices(ib_, jb_, "top").T])==False for ib_,jb_ in unblock_CO])
    ...
    if iteration%2000==0:
        ...
        if iteration>20000:
            ...
            a,b,*ignore = lr(it_list[-10:], D_reactions_list[-10:])
            #a_err=np.sqrt(np.diag(pcov))[0]
            
            ax.plot(it_list,D_reactions_list,label="Reaction pr iteration")
            ax.plot(it_list[-10:],line(np.array(it_list[-10:]),a,b),label="Linear regression")
            ax.set_ylabel("Reactions pr. iteration")
            ax.set_xlabel("Iteration")
            ax.legend()
            FigureCanvas(fig).draw()
            image = np.frombuffer(fig.canvas.tostring_rgb(), dtype='uint8')
            image = image.reshape(fig.canvas.get_width_height()[::-1] + (3,))
            my_gif_frames.append(image)

            # clear ax before next iteration
            ax.clear()
            #camera.snap()
            #ax.clear()
            #plt.show()
            ...
            if abs(a)<(mean_D_reactions*1e-6):
                ...
                break
            #if np.all(abs(mean_D_reactions - D_reactions_list[-5:])/mean_D_reactions < 0.05):
                #break
...
ax.plot(it_list[-10:],line(np.array(it_list[-10:]),a,b))
#ax2=ax.twinx()
#ax2.plot(it_list[1:],D_energies,c="tab:orange")
#ax.set_xscale("log")
print(mean_D_reactions,b)
print(D_reactions_list)
print(abs(mean_D_reactions - D_reactions_list[-5:]))
print(np.std(D_reactions_list[-5:]))


# save your gif
imageio.mimsave('example.gif', my_gif_frames, fps=int(len(my_gif_frames)/gif_length))
