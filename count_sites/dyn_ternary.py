...
import sys
sys.path.append("..")
from methods import count_sites_dynamic as csd
...
for composition in tqdm(grid_mf):
    site1 = np.append(site1,csd(composition, P_CO=1.0, P_NO=1.0))
    site05 = np.append(site05,csd(composition, P_CO=1.0, P_NO=0.5))
...
np.savetxt("AgAuCu_grid_dyn.csv",data,delimiter=",",header="Ag,Au,Cu,active sites (P_NO=1),active sites (P_NO=0.5)",comments="",fmt=["%.2f","%.2f","%.2f","%.6f","%.6f"])
...
np.savetxt("AuCuPd_grid_dyn.csv",data,delimiter=",",header="Au,Cu,Pd,active sites (P_NO=1),active sites (P_NO=0.5)",comments="",fmt=["%.2f","%.2f","%.2f","%.6f","%.6f"])
