...
for composition in tqdm(ternary_grid_mf):
        n_sites1 = np.append(n_sites1,count_sites(composition, P_CO=1.0, P_NO=1.0,metals=metals,method='dyn'))
        n_sites05 = np.append(n_sites05,count_sites(composition, P_CO=1.0, P_NO=0.5,metals=metals,method='dyn'))
        n_sites01 = np.append(n_sites01,count_sites(composition, P_CO=1.0, P_NO=0.1,metals=metals,method='dyn'))


data = np.hstack((ternary_grid_mf,n_sites1.reshape(-1,1),n_sites05.reshape(-1,1),n_sites01.reshape(-1,1)))


np.savetxt('dynamic/ternary_grid.csv',data,delimiter=',',fmt=['%0.2f','%0.2f','%0.2f','%0.2f','%0.2f','%.4f','%.4f','%.4f'],header='Ag,Au,Cu,Pd,Pt,P_NO=1,P_NO=05,P_NO=01')