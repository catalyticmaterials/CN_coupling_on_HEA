...
#while len(CO_data)>0 or len(NO_data)>0:
    ...
    if iteration%1000==0:
        ...
        if len(energies)>1:
            ...
            if abs(D_energies[-1])<0.001:
                break
    #if np.any([NO_coverage_mask==None,CO_coverage_mask==None])==False:
    #if np.any([NO_coverage_mask.flatten()[NO_energy_grid.flatten()<0]==None,CO_coverage_mask.flatten()[CO_energy_grid.flatten()<0]==None])==False:
        #break
...
#Pad grids
NO_coverage_mask_pad = np.pad(NO_coverage_mask,pad_width=((1,1),(1,1)),mode="wrap")
...
for (i,j) in CO_ads_sites:
    if NO_coverage_mask_pad[i,j] == True:
        E_CO = CO_energy_grid[i,j]#CO_energies[i*100+j]
        E_NO = NO_energy_grid_pad[i,j] #NO_energies[(i-1)*100+(j-1)]
        CO_NO_energy_pair = np.vstack((CO_NO_energy_pair,np.array([[E_CO,E_NO]])))
    if NO_coverage_mask_pad[i,j+2] == True:
        E_CO = CO_energy_grid[i,j]#CO_energies[i*100+j]
        E_NO = NO_energy_grid_pad[i,j+2] #NO_energies[(i-1)*100+(j+1)]
        CO_NO_energy_pair = np.vstack((CO_NO_energy_pair,np.array([[E_CO,E_NO]])))
    if NO_coverage_mask_pad[i+2,j] == True:
        E_CO = CO_energy_grid[i,j] #CO_energies[i*100+j]
        E_NO = NO_energy_grid_pad[i+2,j] #NO_energies[(i+1)*100+(j-1)]
        CO_NO_energy_pair = np.vstack((CO_NO_energy_pair,np.array([[E_CO,E_NO]])))
...
len(CO_NO_energy_pair)
print(n_CO_NO_pairs/surface_atoms)
...
#print(np.mean(D_energies[np.array(it_list[1:])>=100000]))
...
print(len(CO_NO_pair.T))



print(np.sum(pairs1_bool.flatten())+np.sum(pairs2_bool.flatten())+np.sum(pairs3_bool.flatten()))
