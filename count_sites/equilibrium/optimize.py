...
for P_NO in [1,0.5,0.1]:
    ...
    n_sites = np.array([count_sites(f, P_CO=1.0, P_NO=P_NO,metals=metals,method='eq') for f in f_train])
    ...
    for i in range(200):
        ...
        n_sites = np.append(n_sites,count_sites(f_next, P_CO=1.0, P_NO=P_NO,metals=metals,method='eq'))
        r_train=molar_fractions_to_cartesians(f_train)
        # Update gpr
        gpr.fit(r_train,n_sites)
    ...
    np.savetxt(f'equilibrium/Bayesian_optimization_results_PNO_{P_NO_str}.csv',data,delimiter=',',fmt=['%.2f','%.2f','%.2f','%.2f','%.2f','%.4f'],header='Ag,Au,Cu,Pd,Pt,active sites')
