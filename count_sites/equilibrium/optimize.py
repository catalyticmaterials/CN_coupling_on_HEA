...
for i in range(150):
    ...
    n_sites = np.append(n_sites,count_sites(f_next, P_CO=1.0, P_NO=1.0,metals=metals,method='eq'))
    r_train=molar_fractions_to_cartesians(f_train)
    # Update gpr
    gpr.fit(r_train,n_sites)
...
np.savetxt('equilibrium/Bayesian_optimization_results_PNO_1.csv',data,delimiter=',',fmt='%.4f',header='Ag,Au,Cu,Pd,Pt,active sites')
