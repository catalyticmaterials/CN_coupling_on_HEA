...
for composition in tqdm(grid_mf):
    ...
    site05 = np.append(site05,dyn_eq(composition, P_CO=1.0, P_NO=0.5))
    sites02 = np.append(sites02,dyn_eq(composition, P_CO=1.0, P_NO=0.2))


data = np.hstack((grid_mf[:,:3],site1.reshape(-1,1),site05.reshape(-1,1),sites02.reshape(-1,1)))


np.savetxt("AgAuCu_grid_dyn_eq.csv",data,delimiter=",",header="Ag,Au,Cu,active sites (P_NO=1),active sites (P_NO=0.5)",comments="",fmt=["%.2f","%.2f","%.2f","%.6f","%.6f","%.6f"])
...
pt.make_plot(grid.T, sites02, ["Ag","Au","Cu"],colorbar=True)
...
data = np.hstack((grid_mf_ternary,site1.reshape(-1,1),site05.reshape(-1,1),sites02.reshape(-1,1)))


np.savetxt("AuCuPd_grid_dyn_eq.csv",data,delimiter=",",header="Au,Cu,Pd,active sites (P_NO=1),active sites (P_NO=0.5)",comments="",fmt=["%.2f","%.2f","%.2f","%.6f","%.6f","%.6f"])
...
pt.make_plot(grid.T, sites02, ["Au","Cu","Pd"],colorbar=True)
