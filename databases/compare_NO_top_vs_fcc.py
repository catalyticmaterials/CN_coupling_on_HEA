...
fcc_energies = []
top_energies = []
with connect("single_element_NO_fcc_out.db") as fcc_db, connect("single_element_NO_angled_out.db") as top_db:
    for (row_fcc,row_top) in zip(fcc_db.select(),top_db.select()):
...
fcc_energies=np.array(fcc_energies)[sort_fcc]
top_energies=np.array(top_energies)[sort_top]
...
for i,metal in enumerate(metals):
    plt.text(fcc_energies[i]+0.01,energy_diff[i]+0.01,s=metal)
plt.tight_layout()
