...
for elements in combinations(metals,3):
    ...
    plt.savefig(f'ternary_plots/{alloy}/{alloy}_mc_PNO_1.png')
    plt.close()

    fig,ax = pt.make_plot(ternary_grid, n_sites05, elements,colorbar=True)
    plt.savefig(f'ternary_plots/{alloy}/{alloy}_mc_PNO_05.png')
    plt.close()

    fig,ax = pt.make_plot(ternary_grid, n_sites01, elements,colorbar=True)
    plt.savefig(f'ternary_plots/{alloy}/{alloy}_mc_PNO_01.png')
    plt.close()
